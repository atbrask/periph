...
// be reinstantiated.
func NewSPI(s spi.ConnConfig, dc gpio.PinOut, w, h int, rotated bool) (*Dev, error) {
	if dc == gpio.INVALID {
	...
	bus, err := s.DevParams(3300000, spi.Mode0, bits)
	if err != nil {
		return nil, err
	}
	return newDev(bus, w, h, rotated, true, dc)
}

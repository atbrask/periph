...
// RecordRaw implements spi.ConnConfigCloser.
//
...
// Record implements spi.ConnConfigCloser that records everything written to it.
//
// This can then be used to feed to Playback to do "replay" based unit tests.
type Record struct {
	sync.Mutex
	Conn        spi.ConnConfigCloser // Conn can be nil if only writes are being recorded.
	Ops         []conntest.IO
	Initialized bool
}
...
// Close implements spi.ConnConfigCloser.
...
// LimitSpeed implements spi.ConnConfigCloser.
...
// DevParams implements spi.ConnConfigCloser.
func (r *Record) DevParams(maxHz int64, mode spi.Mode, bits int) (spi.Conn, error) {
	...
	r.Initialized = true
	if r.Conn != nil {
		c, err := r.Conn.DevParams(maxHz, mode, bits)
		if err != nil {
			return nil, err
		}
...
// Playback implements spi.ConnConfigCloser and plays back a recorded I/O flow.
//
...
// Log logs all operations done on an spi.ConnConfigCloser.
type Log struct {
	Conn spi.ConnConfigCloser
}
...
func (l *Log) DevParams(maxHz int64, mode spi.Mode, bits int) (spi.Conn, error) {
	s, err := l.Conn.DevParams(maxHz, mode, bits)
	log.Printf("%s.DevParams(%d, %d, %d) = %v", l.Conn, maxHz, mode, bits, err)
	return &LogConn{s}, err
}
...
var _ spi.ConnConfigCloser = &RecordRaw{}
var _ spi.ConnConfigCloser = &Record{}
var _ spi.ConnConfigCloser = &Playback{}
var _ spi.ConnConfigCloser = &Log{}
